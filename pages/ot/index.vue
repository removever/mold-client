...
const { getOTBy, deleteOTBy } = useOT();
const { $auth } = useNuxtApp()
const { permission_add, permission_delete, permission_edit } = $auth.getPermission('ot')
...
const headers: Headers = [
  ...
  { title: "เครื่องจักร", key: "machine_name", sortable: false, width: 200, align: 'center' },
  { title: "เริ่ม", key: "ot_start", sortable: false, width: 300, align: 'center' },
  { title: "สิ้นสุด", key: "ot_end", sortable: false, width: 300, align: 'center' },
  { title: "หมายเหตุ", key: "ot_remark", sortable: false, align: 'center' },
];
const columns: any[] = [
  { value: "machine_id", title: "วันที่เริ่ม", },
  { value: "ot_start", title: "วันที่เริ่ม", },
  { value: "ot_end", title: "วันที่สิ้นสุด", },
];
...
const {
  ...
  pending: pendingOTs,
  refresh: refreshOTs,
} = await useAsyncData('ots', () => getOTBy({ pagination: pagination.value, search: search.value }), { watch: [pagination.value] })
...
<template>
  <v-card elevation="10" class="withbg mx-auto" color="grey-lighten-3" max-width="1980">
    <v-card-title>ทำงานล่วงเวลา</v-card-title>
    <v-card-item>
      <v-row class="mb-4">
        <v-col class="d-flex flex-wrap gap-2 align-center py-2" cols="12" md="4">
          <template v-if="permission_add">
            <v-btn elevation="3" color="primary" to="/ot/add">เพิ่มรายการ</v-btn>
          </template>
        </v-col>
        <v-col class="d-flex flex-wrap gap-2 align-center justify-md-end py-2" cols="12" md="8">
          <form class="d-flex flex-wrap gap-2" :onsubmit="submitSearch">
            <v-text-field v-model="search.text" label="Search..." append-inner-icon="mdi-magnify" density="compact"
              variant="outlined" hide-details style="width: 500px" @click:append-inner="refreshOTs()"></v-text-field>
          </form>
      ...
      <v-data-table-server :loading="pendingOTs" :headers="headers" :items="ots?.docs ?? []"
        :items-length="ots?.totalDocs ?? 0" :items-per-page="pagination.size" :page="pagination.page"
        ...
        <template v-slot:item.ot_start="{ item }">
          {{ formatDate(item.raw.ot_start,'dd/MM/yyyy HH:mm') }}
        </template>
        <template v-slot:item.ot_end="{ item }">
          {{ formatDate(item.raw.ot_end,'dd/MM/yyyy HH:mm') }}
        </template>
          ...
          <v-menu offset-y>
            ...
            <v-list>
              <v-list-item class="cursor-pointer" density="compact"
                :to="{ path: '/ot/detail', query: { id: item.raw.ot_id } }">
                <v-list-item-title>
              ...
              <template v-if="permission_edit">
                <v-list-item class="cursor-pointer" density="compact"
                  :to="{ path: '/ot/update', query: { id: item.raw.ot_id } }">
                  <v-list-item-title>
              ...
              <template v-if="permission_delete">
                <v-list-item class="cursor-pointer" density="compact" @click="onDelete(item.raw.ot_id)">
                  <v-list-item-title>
