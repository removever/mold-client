...
const { getProcessTitleBy, deleteProcessTitleBy } = useProcessTitle();
const { $auth } = useNuxtApp()
const { permission_add, permission_delete, permission_edit } = $auth.getPermission('process-title')
...
const headers: Headers = [
  ...
  { title: "รหัสคุณสมบัติ", key: "process_title_id", sortable: false, width: 200, },
  { title: "ชื่อคุณสมบัติ", key: "process_title_name", sortable: false },
];
const columns: any[] = [
  { value: "process_title_id", title: "รหัสคุณสมบัติ", },
  { value: "process_title_name", title: "ชื่อคุณสมบัติ", },
];
...
const {
  data: process_titles,
  pending: pendingProcessTitles,
  refresh: refreshProcessTitles,
} = await useAsyncData('process_titles', () => getProcessTitleBy({ pagination: pagination.value, search: search.value }), { watch: [pagination.value] })
...
const onDelete = (id: string) => Swal.fire({
  ...
  try {
    ...
    await deleteProcessTitleBy({ process_title_id: id })

    refreshProcessTitles()
...
<template>
  <v-card elevation="10" class="withbg mx-auto" color="grey-lighten-3" max-width="1980">
    <v-card-title>คุณสมบัติ</v-card-title>
    <v-card-item>
      <v-row class="mb-4">
        <v-col class="d-flex flex-wrap gap-2 align-center py-2" cols="12" md="4">
          <template v-if="permission_add">
            <v-btn elevation="3" color="primary" to="/process-title/add">เพิ่มรายการใหม่</v-btn>
          </template>
        </v-col>
        <v-col class="d-flex flex-wrap gap-2 align-center justify-md-end py-2" cols="12" md="8">
          <form class="d-flex flex-wrap gap-2" :onsubmit="submitSearch">
            <v-text-field v-model="search.text" label="Search..." append-inner-icon="mdi-magnify" density="compact"
              variant="outlined" hide-details style="width: 500px" @click:append-inner="refreshProcessTitles()"></v-text-field>
          </form>
      ...
      <v-data-table-server :loading="pendingProcessTitles" :headers="headers" :items="process_titles?.docs ?? []"
        :items-length="process_titles?.totalDocs ?? 0" :items-per-page="pagination.size" :page="pagination.page"
          ...
          <v-menu offset-y>
            ...
            <v-list>
              <v-list-item class="cursor-pointer" density="compact"
                :to="{ path: '/process-title/detail', query: { id: item.raw.process_title_id } }">
                <v-list-item-title>
              ...
              <template v-if="permission_edit">
                <v-list-item class="cursor-pointer" density="compact"
                  :to="{ path: '/process-title/update', query: { id: item.raw.process_title_id } }">
                  <v-list-item-title>
              ...
              <template v-if="permission_delete">
                <v-list-item class="cursor-pointer" density="compact" @click="onDelete(item.raw.process_title_id)">
                  <v-list-item-title>
