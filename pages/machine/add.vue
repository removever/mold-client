...
const { generateMachineID, insertMachine, } = useMachine();
...
onMounted(async () => {
  try {
    machine.value.machine_id = await generateMachineID()
        ...
        <v-col cols="12" md="12">
          <v-row>
            ...
            <v-col cols="12" sm="6" lg="4">
              ...
              <v-select class="ml-2" label="แผนก" v-model="machine.department_id" :items="department_options" density="compact" variant="outlined" hide-details></v-select>
            </v-col>
          </v-row>
        </v-col>
      </v-row>
            ...
            <tbody>
              <tr v-if="machine_process_lists.filter(list => list.process_id === item.process_id).length" 
                ...
                <td class="align-middle text-center">
                  <v-text-field v-model="list.machine_process_list_value" :rules="[rules.required]" density="compact" variant="outlined" hide-details></v-text-field>
                </td>
              </tr>
              <tr v-else>
      ...
      <table class="table table-bordered" style="min-width: 400px" aria-describedby="title">
        <thead>
          <tr>
            ...
            <th scope="Time" style="width: 600px">วันที่หยุด</th>
            ...
            <td class="align-middle text-center">
              <v-row>
                <v-col cols="6" sm="12" lg="6">
                  <v-text-field label="เริ่ม" type="datetime-local" v-model="list.machine_stop_start" density="compact" variant="outlined" :rules="[rules.required]"></v-text-field>
                </v-col>
                <v-col cols="6" sm="12" lg="6">
                  <v-text-field label="สิ้นสุด" type="datetime-local" v-model="list.machine_stop_end" density="compact" variant="outlined" :rules="[rules.required]"></v-text-field>
                </v-col>
              </v-row>
            </td>
            <td class="align-middle text-center">
              <v-switch class="ml-5" v-model="list.machine_stop_ot_status" label="เปิด OT" color="primary" hide-details></v-switch>
            </td>
            <td class="align-middle text-center">
              <v-text-field type="text" v-model="list.machine_stop_remark" density="compact" variant="outlined" :rules="[rules.required]"></v-text-field>
            </td>
            <td class="align-middle text-center">
