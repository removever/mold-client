...
const { getMachineBy, deleteMachineBy } = useMachine();
const { $auth } = useNuxtApp()
const { permission_add, permission_delete, permission_edit } = $auth.getPermission('machine')
...
const headers: Headers = [
  ...
  { title: "รหัสเครื่องจักร", key: "machine_id", sortable: false, width: 200, },
  { title: "ชื่อเครื่องจักร", key: "machine_name", sortable: false },
];
const columns: any[] = [
  { value: "machine_id", title: "รหัสเครื่องจักร", },
  { value: "machine_name", title: "ชื่อเครื่องจักร", },
];
...
} = await useAsyncData('machines', () => getMachineBy({ pagination: pagination.value, search: search.value }), { watch: [pagination.value] })
...
const onDelete = (id: string) => Swal.fire({
  ...
  try {
    ...
    await deleteMachineBy({ machine_id: id })
...
<template>
  <v-card elevation="10" class="withbg mx-auto" color="grey-lighten-3" max-width="1980">
    <v-card-title>เครื่องจักร</v-card-title>
    <v-card-item>
      <v-row class="mb-4">
        <v-col class="d-flex flex-wrap gap-2 align-center py-2" cols="12" md="4">
          <template v-if="permission_add">
            <v-btn elevation="3" color="primary" to="/machine/add">เพิ่มรายการใหม่</v-btn>
          </template>
        </v-col>
        <v-col class="d-flex flex-wrap gap-2 align-center justify-md-end py-2" cols="12" md="8">
          <form class="d-flex flex-wrap gap-2" :onsubmit="submitSearch">
            <v-text-field v-model="search.text" label="Search..." append-inner-icon="mdi-magnify" density="compact"
              variant="outlined" hide-details style="width: 500px" @click:append-inner="refreshMachines()"></v-text-field>
          </form>
      ...
      <v-data-table-server :loading="pendingMachines" :headers="headers" :items="machines?.docs ?? []"
        :items-length="machines?.totalDocs ?? 0" :items-per-page="pagination.size" :page="pagination.page"
          ...
          <v-menu offset-y>
            ...
            <v-list>
              <v-list-item class="cursor-pointer" density="compact"
                :to="{ path: '/machine/detail', query: { id: item.raw.machine_id } }">
                <v-list-item-title>
              ...
              <template v-if="permission_edit">
                <v-list-item class="cursor-pointer" density="compact"
                  :to="{ path: '/machine/update', query: { id: item.raw.machine_id } }">
                  <v-list-item-title>
              ...
              <template v-if="permission_delete">
                <v-list-item class="cursor-pointer" density="compact" @click="onDelete(item.raw.machine_id)">
                  <v-list-item-title>
