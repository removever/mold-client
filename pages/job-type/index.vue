...
const { getJobTypeBy, deleteJobTypeBy } = useJobType();
const { $auth } = useNuxtApp()
const { permission_add, permission_delete, permission_edit } = $auth.getPermission('job-type')
...
const headers: Headers = [
  ...
  { title: "ชื่อประเภทงาน", key: "job_type_name", sortable: false },
];
const columns: any[] = [
  { value: "job_type_name", title: "ชื่อประเภทงาน", },
];
...
const {
  data: job_types,
  pending: pendingJobTypes,
  refresh: refreshJobTypes,
} = await useAsyncData('job_types', () => getJobTypeBy({ pagination: pagination.value, search: search.value }), { watch: [pagination.value] })
...
const onDelete = (id: string) => Swal.fire({
  ...
  try {
    ...
    await deleteJobTypeBy({ job_type_id: id })
...
<template>
  <v-card elevation="10" class="withbg mx-auto" color="grey-lighten-3" max-width="1980">
    <v-card-title>ประเภทงาน</v-card-title>
    <v-card-item>
      <v-row class="mb-4">
        <v-col class="d-flex flex-wrap gap-2 align-center py-2" cols="12" md="4">
          <template v-if="permission_add">
            <v-btn elevation="3" color="primary" to="/job-type/add">เพิ่มรายการใหม่</v-btn>
          </template>
        </v-col>
        <v-col class="d-flex flex-wrap gap-2 align-center justify-md-end py-2" cols="12" md="8">
          <form class="d-flex flex-wrap gap-2" :onsubmit="submitSearch">
            <v-text-field v-model="search.text" label="Search..." append-inner-icon="mdi-magnify" density="compact"
              variant="outlined" hide-details style="width: 500px" @click:append-inner="refreshJobTypes()"></v-text-field>
          </form>
      ...
      <v-data-table-server :loading="pendingJobTypes" :headers="headers" :items="job_types?.docs ?? []"
        :items-length="job_types?.totalDocs ?? 0" :items-per-page="pagination.size" :page="pagination.page"
          ...
          <v-menu offset-y>
            ...
            <v-list>
              <v-list-item class="cursor-pointer" density="compact"
                :to="{ path: '/job-type/detail', query: { id: item.raw.job_type_id } }">
                <v-list-item-title>
              ...
              <template v-if="permission_edit">
                <v-list-item class="cursor-pointer" density="compact"
                  :to="{ path: '/job-type/update', query: { id: item.raw.job_type_id } }">
                  <v-list-item-title>
              ...
              <template v-if="permission_delete">
                <v-list-item class="cursor-pointer" density="compact" @click="onDelete(item.raw.job_type_id)">
                  <v-list-item-title>
