...
const { permission_add, permission_delete, permission_edit } = $auth.getPermission('user_position')
...
} = await useAsyncData('user_positions', () => getUserPositionBy({ pagination: pagination.value, search: search.value }), { watch: [pagination.value] })
        ...
        <v-col class="d-flex flex-wrap gap-2 align-center justify-md-end py-2" cols="12" md="8">
          <form class="d-flex flex-wrap gap-2" :onsubmit="submitSearch">
            <v-text-field v-model="search.text" label="Search..." append-inner-icon="mdi-magnify" density="compact"
              variant="outlined" hide-details style="width: 500px" @click:append-inner="refreshUserPositions()"></v-text-field>
          </form>
