...
const { getScheduleBy, deleteScheduleBy } = useSchedule();
const { $auth } = useNuxtApp()
const { permission_add, permission_delete, permission_edit } = $auth.getPermission('schedule')
...
const headers: Headers = [
  ...
  { title: "วันที่", key: "schedule_date", sortable: false, align: 'center' },
];
const columns: any[] = [
  { value: "schedule_start_date", title: "วันที่เริ่ม", },
  { value: "schedule_end_date", title: "วันที่สิ้นสุด", },
];
...
} = await useAsyncData('schedules', () => getScheduleBy({ pagination: pagination.value, search: search.value }), { watch: [pagination.value] })
...
const onDelete = (id: string) => Swal.fire({
  ...
  try {
    ...
    await deleteScheduleBy({ schedule_id: id })
    refreshSchedules()
...
<template>
  <v-card elevation="10" class="withbg mx-auto" color="grey-lighten-3" max-width="1980">
    <v-card-title>เวลาการทำงาน</v-card-title>
    <v-card-item>
      <v-row class="mb-4">
        <v-col class="d-flex flex-wrap gap-2 align-center py-2" cols="12" md="4">
          <template v-if="permission_add">
            <v-btn elevation="3" color="primary" to="/schedule/add">เพิ่มเวลาการทำงาน</v-btn>
          </template>
        </v-col>
        <v-col class="d-flex flex-wrap gap-2 align-center justify-md-end py-2" cols="12" md="8">
          <form class="d-flex flex-wrap gap-2" :onsubmit="submitSearch">
            <v-text-field v-model="search.text" label="Search..." append-inner-icon="mdi-magnify" density="compact"
              variant="outlined" hide-details style="width: 500px" @click:append-inner="refreshSchedules()"></v-text-field>
          </form>
      ...
      <v-data-table-server :loading="pendingSchedules" :headers="headers" :items="schedules?.docs ?? []"
        :items-length="schedules?.totalDocs ?? 0" :items-per-page="pagination.size" :page="pagination.page"
        ...
        <template v-slot:item.schedule_date="{ item }">{{ `${formatDate(item.raw.schedule_start_date)} - ${formatDate(item.raw.schedule_end_date)}` }}</template>
          ...
          <v-menu offset-y>
            ...
            <v-list>
              <v-list-item class="cursor-pointer" density="compact"
                :to="{ path: '/schedule/detail', query: { id: item.raw.schedule_id } }">
                <v-list-item-title>
              ...
              <template v-if="permission_edit">
                <v-list-item class="cursor-pointer" density="compact"
                  :to="{ path: '/schedule/update', query: { id: item.raw.schedule_id } }">
                  <v-list-item-title>
              ...
              <template v-if="permission_delete">
                <v-list-item class="cursor-pointer" density="compact" @click="onDelete(item.raw.schedule_id)">
                  <v-list-item-title>
